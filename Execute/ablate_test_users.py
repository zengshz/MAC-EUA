...
def ablate_test_users():
    users_num_list = [1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000]
    ...
    for index, user_num in enumerate(users_num_list):
        print(f"\n===== 处理用户数: {user_num}（{index + 1}/{len(users_num_list)}） =====")
        ...
        for group_idx, (servers, users, connect, latency) in tqdm(
            ...
            elapsed_ms_not, (_, _, _, not_allocated_user_ratio, _, not_capacity_used_ratio, not_propagation_delay_aver) = measure_gpu_time(
            ...
            elapsed_ms_location, (_, _, _, location_allocated_user_ratio, _, location_capacity_used_ratio, location_propagation_delay_aver) = measure_gpu_time(
            ...
            elapsed_ms_encoder, (_, _, _, encoder_allocated_user_ratio, _, encoder_capacity_used_ratio, encoder_propagation_delay_aver) = measure_gpu_time(
        ...
        result_df.insert(1, 'servers', servers0.size(1))  # 添加服务器数量列
    ...
    save_path = os.path.join(
        result_dir,
        f"ablate_test_{server_percent}servers_{save_time}.csv"
    )
