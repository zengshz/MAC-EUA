...
def baseline_normal_user():
    users_num_list = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]
    ...
    dsam_state_dict = torch.load(
        '../Analyze/model/dsam/11161819_server_50_user_800_mean_35_std_10/11170745_65.75_98.58_62.77.mdl',
        map_location='cpu')
    ...
    save_path = os.path.join(
        result_dir,
        f"baseline_small_{server_percent}servers_{save_time}.csv"
    )
    final_df.to_csv(save_path, index=False)
    print(f"\n基线算法结果已保存至：{save_path}")


# -------------------------- 执行入口 --------------------------
if __name__ == '__main__':
    if torch.cuda.is_available():
        baseline_normal_user()
    else:
