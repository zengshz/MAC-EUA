...
class Encoder(nn.Module):
    ...
    def __init__(self, d_model, dropout, edge_dim, user_feature_dim, server_feature_dim, spatial_raw_dim):
        super().__init__()
    ...
    def forward(self, users, servers):
        ...
        # 3. Project edge_emb to d_model for decoder
...
# ----------------------------
class MACAllocator_Ablate_Location(nn.Module):
    """
        输入张量规格：
            servers: [B, S, 7]  (X, Y, RADIUS, cpu, ram, storage, bandwidth)
            users:   [B, U, 6]  (X, Y, cpu, ram, storage, bandwidth)
    ...
    def forward(self, servers, users, connect, p_distance):
        ...
        user_enc, server_enc, spatial_enc = self.encoder(users=users, servers=servers)
