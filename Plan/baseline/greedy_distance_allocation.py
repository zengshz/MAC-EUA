...
# 490  495
def greedy_distance_allocation(servers, users, connect, latency):
    '''
    :param servers:  [B, S, 7] (服务器特征) LATITUDE,LONGITUDE,RADIUS,CPU,RAM,BANDWIDTH,STORAGE
    :param users: [B, U, 6] (用户特征) LATITUDE,LONGITUDE,CPU,RAM,BANDWIDTH,STORAGE
    :param connect: [B, U, S] (连接矩阵) 边缘服务器的覆盖用户信息，行为用户，列为服务器，总长度为用户数*服务器数，数据类型设置为布尔值
    :param latency: [B, U, S] (传播延迟矩阵） 行为用户，列为服务器，存储有用户到该服务器的基于距离计算的传播延迟值
    :return:
    '''
    # 提取批次信息
    ...
    for i in range(num_users):
        ...
        for b in range(batch_size):
            ...
            if server_ids.numel() > 0:
                # fetch latency for this user b
                lat_vals = latency[b, i, server_ids]  # [n]
                # score: lower is better
                score = lat_vals
                idx = torch.argmin(score)
                valid_servers.append(server_ids[idx])
            else:
