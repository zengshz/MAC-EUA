...
def random_allocation(servers, users, connect, latency):
    ...
    masked_cap = remain_capacity.masked_fill(~server_allocated_user_flag.unsqueeze(-1).expand(batch_size, num_servers, 4), value=0)  # 剩余容量，形状[B, S, 4]
    masked_servers = servers[:, :, 3:7].masked_fill(~server_allocated_user_flag.unsqueeze(-1).expand(batch_size, num_servers, 4),
